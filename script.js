...
// ==========================================
// STRICT IMAGE PICKER HELPER
// ==========================================

async function openImagePicker(inputId, callback) {
...
function validateImageFile(file) {
    const validImageTypes = ['image/png', 'image/jpeg', 'image/jpg', 'image/webp', 'image/gif', 'image/avif', 'image/svg+xml'];
    const validExtensions = ['.png', '.jpg', '.jpeg', '.webp', '.gif', '.avif', '.svg'];
    const fileName = file.name.toLowerCase();
    const hasValidExtension = validExtensions.some(ext => fileName.endsWith(ext));
    const hasValidType = validImageTypes.includes(file.type) || file.type.startsWith('image/');

    return {
        valid: hasValidType || hasValidExtension,
        name: fileName
    };
}
...
// Callback wrappers for the strict image picker
function handleAvatarFile(file) {
    const validation = validateImageFile(file);
    if (!validation.valid) {
        showToast('Please select an image file (PNG, JPG, WebP, or GIF)');
        return;
    }
    ...
    reader.onerror = function() {
        showToast('Error reading image. Please try again.');
    };
...
function handleEditAvatarFile(file) {
    ...
    reader.onload = function(e) {
        ...
        if (editAvatarImg) editAvatarImg.src = imageData;
        updateAllAvatars(imageData);
        const savedProfile = JSON.parse(localStorage.getItem('userProfile') || '{}');
        savedProfile.hasCustomAvatar = true;
...
function handleAvatarUpload(event) {
    ...
    reader.onload = function(e) {
        ...
        // Update all avatar images in the app
        updateAllAvatars(imageData);
...
function updateAllAvatars(imageData) {
    ...
    // Update header avatar (upper left)
    const headerAvatar = document.getElementById('header-avatar-img');
    if (headerAvatar) headerAvatar.src = imageData;

    // Update local video avatar in call screen
    const callAvatar = document.querySelector('#call-screen .local-video img');
    if (callAvatar) callAvatar.src = imageData;

    // Update edit profile modal avatar
    const editAvatar = document.getElementById('edit-avatar-img');
    if (editAvatar) editAvatar.src = imageData;

    // NOTE: auth-logo-img is the APP LOGO, not user avatar - do NOT update it!
}

function loadSavedAvatar() {
    const savedAvatar = localStorage.getItem('userAvatar');
    if (savedAvatar) {
        // Only update avatar images, NOT the app logo
        const profileAvatar = document.getElementById('profile-avatar-img');
        ...
        const callAvatar = document.querySelector('#call-screen .local-video img');
        if (callAvatar) callAvatar.src = savedAvatar;
...
function showPaymentView() {
    ...
    document.querySelectorAll('.radio-circle').forEach(r => r.style.background = '');
    hideAllPaymentAreas();
    document.getElementById('payment-confirm-btn').classList.remove('hidden');
}
...
function selectPaymentMethod(method) {
    ...
    // Show relevant payment area
    hideAllPaymentAreas();

    if (method === 'paypal') {
        document.getElementById('payment-confirm-btn').classList.add('hidden');
        const paypalContainer = document.getElementById('paypal-button-container');
        paypalContainer.classList.remove('hidden');
        renderPayPalButton();
    } else if (method === 'gcash') {
        document.getElementById('payment-confirm-btn').classList.add('hidden');
        document.getElementById('gcash-payment-area').classList.remove('hidden');
    } else if (method === 'paymaya') {
        document.getElementById('payment-confirm-btn').classList.add('hidden');
        document.getElementById('paymaya-payment-area').classList.remove('hidden');
    }
}

function hideAllPaymentAreas() {
    document.getElementById('paypal-button-container').classList.add('hidden');
    document.getElementById('gcash-payment-area').classList.add('hidden');
    document.getElementById('paymaya-payment-area').classList.add('hidden');
}
...
function renderPayPalButton() {
    ...
    if (typeof paypal === 'undefined') {
        container.innerHTML = '<p style="color: var(--gray); text-align: center;">PayPal SDK loading...</p>';
        return;
    }
    ...
    try {
        paypal.Buttons({
            ...
            createOrder: function(data, actions) {
                return actions.order.create({
                    purchase_units: [{
                        ...
                        description: 'Learn O\'Clock Premium Subscription'
                    }]
                });
            },
            onApprove: function(data, actions) {
                return actions.order.capture().then(function(details) {
                    ...
                    updateToPremium();
                    setTimeout(() => showView('profile'), 2000);
                });
            },
        ...
        container.innerHTML = '<p style="color: var(--danger); text-align: center;">Unable to load PayPal. Please try another method.</p>';
    }
}

function confirmManualPayment(method) {
    const methodName = method === 'gcash' ? 'GCash' : 'Maya';
    showToast(`${methodName} payment confirmed! Processing...`);

    // Simulate processing
    setTimeout(() => {
        showToast('Payment verified! Welcome to Premium!');
        updateToPremium();
        setTimeout(() => showView('profile'), 2000);
    }, 1500);
}
...
function updateToPremium() {
    // Update UI to show premium status
    const subLabel = document.querySelector('.setting-item .setting-info small');
    if (subLabel && subLabel.textContent.includes('Active')) {
        subLabel.textContent = 'Active • Premium Member';
    }

    // Add XP bonus for subscribing
    gainXP(500);
...
function joinSession(tutorName) {
    ...
    if (avatarImg) avatarImg.src = tutor.avatar || `https://api.dicebear.com/9.x/avataaars/svg?seed=${tutor.seed}&backgroundColor=5A82B4`;
    if (nameEl) nameEl.textContent = tutor.name;
    if (chatAvatar) chatAvatar.src = tutor.avatar || `https://api.dicebear.com/9.x/avataaars/svg?seed=${tutor.seed}&backgroundColor=5A82B4`;
    ...
    setTimeout(() => {
        ...
        const replyHTML = `
            <div class="message received">
                <img src="https://randomuser.me/api/portraits/men/1.jpg" alt="Tutor">
                <div class="message-content">
                    <p>${randomReply}</p>
...
function openAddSessionModal(tutorName) {
    ...
    if (tutorName && tutorData[tutorName]) {
        ...
        // Update display
        document.getElementById('selected-tutor-img').src = `https://randomuser.me/api/portraits/men/1.jpg`;
...
function openEditProfileModal() {
    ...
    // Load avatar - use custom photo if available, otherwise use saved seed
    const customAvatar = localStorage.getItem('userAvatar');
    if (customAvatar && savedProfile.hasCustomAvatar !== false) {
        document.getElementById('edit-avatar-img').src = customAvatar;
    } else if (savedProfile.avatarSeed) {
        currentAvatarSeed = savedProfile.avatarSeed;
        document.getElementById('edit-avatar-img').src = `https://api.dicebear.com/9.x/avataaars/svg?seed=${savedProfile.avatarSeed}&backgroundColor=b6e3f4,c0aede,d1d4f9,ffdfbf`;
    }

    // Hide avatar options grid by default
...
function selectAvatarSeed(seed) {
    ...
    document.getElementById('edit-avatar-img').src = `https://api.dicebear.com/9.x/avataaars/svg?seed=${seed}&backgroundColor=b6e3f4,c0aede,d1d4f9,ffdfbf`;
    document.getElementById('avatar-options').classList.remove('show');

    // Clear custom avatar since user picked a preset avatar
    localStorage.removeItem('userAvatar');
    const savedProfile = JSON.parse(localStorage.getItem('userProfile') || '{}');
    savedProfile.hasCustomAvatar = false;
    localStorage.setItem('userProfile', JSON.stringify(savedProfile));
}
...
function handleEditProfile(e) {
    ...
    // Check if a custom photo was uploaded
    const customAvatar = localStorage.getItem('userAvatar');
...
function updateProfileUI(profile) {
    ...
    // Use custom uploaded photo if available, otherwise use Dicebear avatar
    const customAvatar = localStorage.getItem('userAvatar');
    let avatarUrl;
    if (customAvatar && profile.hasCustomAvatar !== false) {
        avatarUrl = customAvatar;
    } else {
        avatarUrl = `https://api.dicebear.com/9.x/avataaars/svg?seed=${profile.avatarSeed || 'Alex'}&backgroundColor=b6e3f4,c0aede,d1d4f9,ffdfbf`;
    }
    ...
    // Update bio on profile page if bio element exists
    // (Note: profile page doesn't show bio directly, but it's saved)
...
function loadSavedProfile() {
    ...
    if (savedProfile.name) {
        // Restore currentAvatarSeed for avatar picker
...
function handleEditAvatarUpload(event) {
    ...
    reader.onload = function(e) {
        ...
        // Update edit modal preview immediately
        const editAvatarImg = document.getElementById('edit-avatar-img');
        if (editAvatarImg) editAvatarImg.src = imageData;
        ...
        // Mark that we're using a custom avatar
        const savedProfile = JSON.parse(localStorage.getItem('userProfile') || '{}');
        savedProfile.hasCustomAvatar = true;
        localStorage.setItem('userProfile', JSON.stringify(savedProfile));
...
window.handleEditAvatarFile = handleEditAvatarFile;
